...
def main():
    ...
    parser.add_argument("--files", action="store_true", help="仅下载文件（暂不支持）")
    ...
    if args.files:
        logger.warning("文件下载暂不支持，需要先通过抓包确认下载 URL 格式")
        if not (args.images or args.articles):
            return
    ...
    if not images and not articles:
        print("没有待下载的资源")
        conn.close()
        return
